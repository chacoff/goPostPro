...
// Path to database
const DATABASE_PATH string = "C:/Users/VERBRUTH/Files/Post_Pro_Tr2/goPostProTrain2/database/processed.db"
...
var (
	...
	TEMPERATURE_THRESHOLD_FACTOR  float64 = 0.6
...
func threshold_crop_line(measurement_string_array []string) ([]float64, error) {
	...
	float_temperatures_array := make([]float64, len(measurement_string_array))
	if len(measurement_string_array) < 1 { // Case : empty string array
		return float_temperatures_array, nil
	}
	...
	for index, temperature_string := range measurement_string_array {
		...
		if parse_error != nil { // Case : Parsing error
			return float_temperatures_array, parse_error
		}
		if index == 0 {
		...
		float_temperatures_array[index] = temperature_float
	}
	// Implemented ADAPTATIVE VERSION but strange results on empty lines
	//threshold_used := min_temperature*(1-TEMPERATURE_THRESHOLD_FACTOR) + max_temperature*TEMPERATURE_THRESHOLD_FACTOR
	...
	for index, temperature_float := range float_temperatures_array {
		//ADAPTATIVE VERSION
		//thresholded_float := math.Max(threshold_used, temperature_float)
		thresholded_float := math.Max(TEMPERATURE_THRESHOLD, temperature_float)
		// Draw on image
		//ADAPTATIVE VERSION
		//color_level := uint8(255 - (thresholded_float-min_temperature)*255/(max_temperature-min_temperature))
		//FINAL_IMAGE.Set(index, LINE_COUNT, color.RGBA{255, color_level, color_level, 255})
	...
	result := thresholded_temperatures_array[lower_index_crop : higher_index_crop+1]
	if len(result) > int(WIDTH_LIMIT) {
...
// To create the database object to do the querries
func open_database(database_file string) (*sql.DB, error) {
	database, err := sql.Open("sqlite3", database_file)
	return database, err
}

// Show the content of our database
func show_database(database *sql.DB) error {
	// Query
	rows, err := database.Query("SELECT * FROM Measures")
	if err != nil {
		return err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&timestamp, &tr1_max, &tr1_mean, &web_mean, &web_min, &tr3_max, &tr3_mean, &width, &threshold, &filename); err != nil {
			return err
		}
		fmt.Println(timestamp, " : ", tr1_max, " | ", tr1_mean, " | ", web_mean, " | ", tr3_max, " | ", tr3_mean, " | ", width, " | ", threshold, " | ", filename)
	}
...
// To insert a line in the database
func insert_line(database *sql.DB, timestamp string, tr1_max int, tr1_mean int, web_mean int, web_min int, tr3_max int, tr3_mean int, width int, threshold int, filename string) error {
	// Prepare the query
	stmt, err := database.Prepare("INSERT INTO Measures(Timestamp, Tr1_Max, Tr1_Mean, Web_Mean, Web_Min, Tr3_Max, Tr3_Mean, Width, Threshold, Filename) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
	defer stmt.Close()

	// Execute it with the given values
	_, err = stmt.Exec(timestamp, tr1_max, tr1_mean, web_mean, web_min, tr3_max, tr3_mean, width, threshold, filename)
	if err != nil {
		panic(err)
	}

	fmt.Println("Line successfully added")
	return nil
}
...
func main() {
	...
	*/
	...
	database, _ := open_database(DATABASE_PATH)
	err := show_database(database)
	fmt.Println(err)
	insert_line(database, "test", 1, 3, 2, 4, 5, 6, 10, 8, "filename.txt")
